...
void WaveManager::loadWaveDataAsync() {
    ...
    waveLoaderFuture = std::async(std::launch::async, [this]() {
        std::this_thread::sleep_for(std::chrono::milliseconds(800)); // L ejsza symulacja na potrzeby test w
        std::lock_guard<std::mutex> lock(waveMutex);
...
void WaveManager::updateEnemies(float dt, PlayerStats& playerStats) {
    ...
    // --- SPAWNOWANIE WROG W ZE ZMIENIONYM BALANSEM MAP ---
    if (enemiesToSpawn > 0) {
        spawnTimer += dt;
        if (spawnTimer >= 1.0f) {
            std::lock_guard<std::mutex> lock(waveMutex);
            ...
            if (enemiesToSpawn == 1 && currentWave % 5 == 0) {
                ...
                // SKALOWANIE DLA TRUDNEJ MAPY (Ci cy wrogowie od pocz tku)
                ...
                // SKALOWANIE DLA  REDNIEJ MAPY (Balans standardowy)
